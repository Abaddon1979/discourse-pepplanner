...
export default class PepplannerCalculator extends Component {
    ...
    loadSettings() {
        ...
        const saved = this.currentUser.custom_fields['pepplanner_calculator_settings'];
        if (saved) {
            try {
                const settings = typeof saved === 'string' ? JSON.parse(saved) : saved;
                if (settings.syringeSize) this.syringeSize = settings.syringeSize;
                if (settings.peptideAmount) this.peptideAmount = settings.peptideAmount;
                if (settings.waterAmount) this.waterAmount = settings.waterAmount;
                if (settings.desiredDosage) this.desiredDosage = settings.desiredDosage;
            } catch (e) {
                console.error("Pepplanner: Failed to load settings", e);
            }
        }
    }
    ...
    saveSettings() {
        ...
        const settings = {
            ...
            peptideAmount: this.peptideAmount,
            waterAmount: this.waterAmount,
            desiredDosage: this.desiredDosage
        };

        // Update the custom field
        this.currentUser.custom_fields['pepplanner_calculator_settings'] = JSON.stringify(settings);
